...
Ext.onReady(function() {
	...
	var storeKd=Ext.create('Ext.mod.store.LDKd',{
	...
	var rightClickGroup=Ext.create('Ext.menu.Menu',{
		...
		items:[{
			...
			handler:function(btn){
				var grid=Ext.getCmp(id+'grid_kd').getStore();
				grid.reload();
	...
	var storeMedInfo=Ext.create('Ext.mod.store.Med_infoD',{
	...
	var win = Ext.create('Ext.window.Window', {
		title: 'KdTx - აღრიცხვაზე მყოფ თირკმელგადანერგილ პაციენტთა სია',
		closable: false,
		/*
		 width: 350,
		 height:400,
	...
	storeKd.on('load', function(store, records, successful){
		win.setTitle('KdTx - აღრიცხვაზე მყოფ თირკმელგადანერგილ პაციენტთა სია - სულ ' +  store.getTotalCount());
	});
	...
	function CreateEditPatient(data){
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'ლაბოროტორია_Kd_' + data.id + '.xlsx'
						});
					}
				}
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'ინსტრუმენტალური_Kd_' + data.id + '.xlsx'
						});
					}
				}
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'მკურნალობა_Kd_' + data.id + '.xlsx'
						});
					}
				}
		...
		Ext.Ajax.request({
			...
			success: function (response) {
				 ...
				 */
				categories.add(new Ext.form.Label({
					html: '<div><div style="float:left">' + result[0].prefix + " დონორი - " + result[0].category_name + ';</div>' +
...

						{
							...
							handler : function(){
								...
								grid.doExcelExport({
									...
									startCell: 'B8',
									templatefile: "temp.xlsx",
									destinationfile: 'იმუნოსუპრესია_Kd_' + data.id + '.xlsx'
								});
							}
						},
		...
		frmShowPatient= new Ext.window.Window({
			...
			x: 0,
			title: "KdTx " + data.id + " - " + data.patientname + " " + data.patientlastname,
			constrain: true,
			renderTo: Ext.getBody(),
			closable: true,
			autoScroll: true,
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'სამედიცინო ინფორმაცია_Kd_' + data.id + '.xlsx'
														});
													}
												}
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'დიალიზის ინფორმაცია_Kd_' + data.id + '.xlsx'
														});
													}
												}
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'ავადმყოფობის ისტორია_Kd_' + data.id + '.xlsx'
														});
													}
												}
