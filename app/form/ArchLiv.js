...
requires:[
	...
	var storeArchLiv=Ext.create('Ext.mod.store.ArchLiv',{
		model: 'Ext.mod.model.ArchLiv',
		storeCount: 0,
		//buffered: true,
		...
		var rightClickGroup=Ext.create('Ext.menu.Menu',{
			...
			items:[{
				...
				handler:function(btn){
					var grid=Ext.getCmp(id+'grid_archliv').getStore();
						grid.reload();
	...
	combo.on('select', function(combo, records) {
		...
		storeArchLiv.pageSize = p;
		var grid=Ext.getCmp(id+'grid_archliv').getStore();
		grid.reload();
		storeArchLiv.loadPage(1);
	}, this);
	...
	var win = Ext.create('Ext.window.Window', {
            title: 'ArchLiv - არქივი ღვიძლი პაციენტთა სია',
            closable: false,
			/*
            width: 350,
			height:400,
...
*/
                    ...
                    */
					...
					listeners : {
    					itemdblclick: function(dv, record, item, index, e) {
							var grid = Ext.getCmp(id+'grid_archliv');
	...
	storeArchLiv.on('load', function(store, records, successful){
		win.setTitle('ArchLiv - არქივი ღვიძლი პაციენტთა სია - სულ ' +  store.getTotalCount());
	});
	...
	function CreateEditPatient(data){
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'ლაბოროტორია_ArchLiv_' + data.id + '.xlsx'
						});
					}
				}
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'ინსტრუმენტალური_ArchLiv_' + data.id + '.xlsx'
						});
					}
				}
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'მკურნალობა_ArchLiv_' + data.id + '.xlsx'
						});
					}
				}
...

						{
							...
							handler : function(){
								...
								grid.doExcelExport({
									...
									startCell: 'B8',
									templatefile: "temp.xlsx",
									destinationfile: 'იმუნოსუპრესია_ArchLiv_' + data.id + '.xlsx'
								});
							}
						},
		...
		frmShowPatient= new Ext.window.Window({
			...
			x: 0,
			title: "ArchLiv " + data.id + " - " + data.patientname + " " + data.patientlastname,
			constrain: true,
			renderTo: Ext.getBody(),
			closable: true,
			autoScroll: true,
...

									{
										...
										handler : function(){
											...
											grid.doExcelExport({
												...
												startCell: 'B8',
												templatefile: "temp.xlsx",
												destinationfile: 'სამედიცინო ინფორმაცია_ArchLiv_' + data.id + '.xlsx'
											});
										}
									}
...

										{
											...
											handler : function(){
												...
												grid.doExcelExport({
													...
													startCell: 'B8',
													templatefile: "temp.xlsx",
													destinationfile: 'ავადმყოფობის ისტორია_ArchLiv_' + data.id + '.xlsx'
												});
											}
										}
