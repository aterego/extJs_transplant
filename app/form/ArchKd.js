...
Ext.onReady(function() {
	...
	var storeArchKd=Ext.create('Ext.mod.store.ArchKd',{
	...
	var rightClickGroup=Ext.create('Ext.menu.Menu',{
		...
		items:[{
			...
			handler:function(btn){
				var grid=Ext.getCmp(id+'grid_archkd').getStore();
				grid.reload();
	...
	combo.on('select', function(combo, records) {
		...
		storeArchKd.pageSize = p;
		var grid=Ext.getCmp(id+'grid_archkd').getStore();
		grid.reload();
		storeArchKd.loadPage(1);
	}, this);
	...
	var storeDial= Ext.create('Ext.data.Store', {
		fields: ['dialysis_id', 'name'],
		data : [
			{"dialysis_id":70, "name":"პერიტონიალური"},
			{"dialysis_id":71, "name":"ჰემოდიალიზი"}
		]
	});
	...
	var win = Ext.create('Ext.window.Window', {
		title: 'ArchKd - არქივი თირკმელი  სია',
		closable: false,
		/*
		 width: 350,
		 height:400,
		...
		items:[{
			...
			listeners : {
				itemdblclick: function(dv, record, item, index, e) {
					var grid = Ext.getCmp(id+'grid_archkd');
			...
			tbar:[
				...
				/*
				 ...
				 handler:function(btn){
				 var grid = Ext.getCmp(id+'grid_kd');
				 var model = grid.getSelectionModel().getSelection();
				 if(model[0].data != null){
				 model_edit=model[0].data;
	...
	storeArchKd.on('load', function(store, records, successful){
		win.setTitle('ArchKd - არქივი თირკმელი პაციენტთა სია - სულ ' +  store.getTotalCount());
	});
	...
	function CreateEditPatient(data){
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'ლაბოროტორია_ArchKd_' + data.id + '.xlsx'
						});
					}
				}
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'ინსტრუმენტალური_ArchKd_' + data.id + '.xlsx'
						});
					}
				}
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'მკურნალობა_ArchKd_' + data.id + '.xlsx'
						});
					}
				}
		...
		Ext.Ajax.request({
			...
			success: function (response) {
				 ...
				 */
				categories.add(new Ext.form.Label({
					html: '<div><div style="float:left">' + result[0].prefix + " - " + result[0].category_name + ';</div>' +
					...
					//'<div style="float:right; margin-right:5px">აღრიცხვიდან მოხსნის თარიღი ' + result[0].date + '</div>' +

					'<div style="float:right; margin-right:15px"><a class="x-btn x-unselectable x-btn-default-small x-icon x-btn-icon x-btn-default-small-icon" id="edit_date" style="width: 16px; height: 16px; " hidefocus="on" unselectable="on" tabindex="0" href="#" onClick="javascript:OpenRemovalEdit(' + data.id + ',' + data.removal_id + ',\'' + data.removal_date + '\');"><span role="presentation" class="x-btn-wrap" unselectable="on" style="height: 10px;"><span class="x-btn-button" role="presentation" style="height: 10px;"><span class="x-btn-inner x-btn-inner-center" unselectable="on" style="line-height: 16px;">&nbsp;</span><span role="presentation" class="x-btn-icon-el  " unselectable="on" style="background-image:url(../../resources/images/edit3.png);">&nbsp;</span></span></span></a></div>' +
					'<div style="float:right; margin-right:5px">აღრიცხვიდან მოხსნის მიზეზი და თარიღი : <strong>' + data.removal + '</strong>, ' + data.removal_date + '</div>' +
		...
		//***AVA*** add parameter to dialysis store
		storeDialysis.load({
			params: {
				param_id : data.id,
				start: 0, limit: 15
...

						{
							...
							handler : function(){
								...
								grid.doExcelExport({
									...
									startCell: 'B8',
									templatefile: "temp.xlsx",
									destinationfile: 'იმუნოსუპრესია_ArchKd_' + data.id + '.xlsx'
								});
							}
						},
		...
		this.OpenRemovalEdit = function(patient_id, removal_id, date){
			...
			var frmRemovalEdit= new Ext.window.Window({
				//height: 400,
				autoHeight: true,
				width: 550,
				title: 'მოხსნა',
				...
				items:[{
						...
						items:[,{
							...
							items: [
...

								{
									...
									listeners: {
										afterrender: function() {
											comboRemovals.setValue(removal_id);
										}
									}
								}
								,
					...
					buttons:[{
						...
						handler:function(btn){
							...
							if(form.isValid()){
								form.submit({
									url:'../../server/list/patientremovals.php?act=Update',
									method:'post',
									params:{
										param_id:patient_id,
		...
		//***AVA*** context menu (dialysis)
		var rightClickDialysis=Ext.create('Ext.menu.Menu',{
			//height:120,
			autoHeight: true,
			width:200,
			items:[{
				...
				icon: '../../resources/images/refresh.png',
				handler:function(btn){
					var grid=Ext.getCmp('dial_info').getStore();
					grid.reload();
				}
			},{
				text:'Add',
				...
				icon: '../../resources/images/edit.png',
				handler:function(btn){
					var grid = Ext.getCmp('dial_info');
					var model = grid.getSelectionModel().getSelection();
					if(model[0].data != null){
						model_edit=model[0].data;
						CreateEditDialysis(model_edit);
					}
				}
			},{
				text:'Delete',
					...
					if (model.length > 0) {
						Ext.Msg.show({
							...
							fn: function (btn) {
								if (btn == 'yes') {
									Ext.Ajax.request({
										url: '../../server/list/dialysis.php?act=Delete',
										method: 'POST',
										params:{
											param_id:model[0].data.patient_id,
											fc:model[0].data.fc
		...
		frmShowPatient= new Ext.window.Window({
			...
			x: 0,
			title: "ArchKd " + data.id + " - " + data.patientname + " " + data.patientlastname,
			constrain: true,
			renderTo: Ext.getBody(),
			closable: true,
			autoScroll: true,
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'სამედიცინო ინფორმაცია_ArchKd_' + data.id + '.xlsx'
														});
													}
												}
											...
											viewConfig: {
												...
												listeners : {
													itemcontextmenu: function(view, record, item, index,          e){
														e.stopEvent();
														if(acc!='r')
															rightClickDialysis.showAt(e.getXY());
													}
												}
											},
											tbar:[{
												...
												listeners: {
													beforerender: function() {
														if(acc == 'r') {
															var button = Ext.getCmp('btn-c-tambah-dial');
															button.hide();
														}
													},
													afterrender: function() {
														Ext.create('Ext.tip.ToolTip',{
															target:'btn-c-tambah-dial',
															html: 'ახალი დიალიზი',
															anchor: 'top'
														});
													}
												...
												listeners: {
													beforerender: function() {
														if(acc == 'r') {
															var button = Ext.getCmp('btn-c-edit-dial-info');
															button.hide();
														}
													},
													afterrender: function() {
														Ext.create('Ext.tip.ToolTip',{
															target:'btn-c-edit-dial-info',
															html: 'რედაქტირება',
															anchor: 'top'
														});
													}
												...
												listeners: {
													beforerender: function() {
														if(acc == 'r') {
															var button = Ext.getCmp('btn-c-hapus-dial');
															button.hide();
														}
													},
													afterrender: function() {
														Ext.create('Ext.tip.ToolTip',{
															target:'btn-c-hapus-dial',
															html: 'წაშლა',
															anchor: 'top'
														});
													}
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'დიალიზის ინფორმაცია_ArchKd_' + data.id + '.xlsx'
														});
													}
												}
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'ავადმყოფობის ისტორია_ArchKd_' + data.id + '.xlsx'
														});
													}
												}
	...
	function CreateAddDialysis(patient_id){
		var me=this;

		var comboDial = new Ext.form.ComboBox({
			name : 'dialysis_id',
			width: 260,
			store: storeDial,
			mode : 'local',
			...
			triggerAction : 'all',
			displayField  : 'name',
			valueField    : 'dialysis_id',
			editable      : false,
			forceSelection: true,
		});

		var frmAddDialysis= new Ext.window.Window({
			//height: 400,
			autoHeight: true,
			width: 550,
			title: 'ახალი დიალიზი',
			...
			items:[{
				...
				buttons:[{
					...
					icon: '../../resources/images/ok.png',
					handler:function(btn){
						var form = Ext.getCmp('add_dial-info').getForm();
						if(form.isValid()){
							form.submit({
								url:'../../server/list/dialysis.php?act=Add',
								method:'post',
								params:{
									param_id:patient_id
								},
	...
	function CreateEditDialysis(data){
		var me=this;
		...
		var frmEditDialysis= new Ext.window.Window({
			//height: 400,
			autoHeight: true,
			width: 550,
			title: 'დიალიზი',
			...
			items:[{
					...
					items:[,{
						...
						items: [
							...
							},
							{
								listeners: {
									afterrender: function() {
										comboDial.setValue(data.dialysis_id);
									}
								}
							}
						]
				...
				buttons:[{
					...
					icon: '../../resources/images/ok.png',
					handler:function(btn){
						var form = Ext.getCmp('edit_dial-info').getForm();
						if(form.isValid()){
							form.submit({
								url:'../../server/list/dialysis.php?act=Update',
								method:'post',
								params:{
									param_id:data.patient_id,
									fc:data.fc
