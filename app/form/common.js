	...
	function CreateEditPatient(data){
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							destinationfile: 'ლაბოროტორია_Search_' + data.id + '.xlsx'
						});
					}
				}
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'ინსტრუმენტალური_Search_' + data.id + '.xlsx'
						});
					}
				}
		...
		Ext.Ajax.request({
			...
			success: function (response) {
				...
				if ( result[0].prefix == 'LDKd' || result[0].prefix == 'LDLiv' || result[0].prefix == 'LDHTx' )
					str = " დონორი - ";
...

						{
							...
							handler : function(){
								...
								grid.doExcelExport({
									...
									startCell: 'B8',
									templatefile: "temp.xlsx",
									destinationfile: 'იმუნოსუპრესია_Search_' + data.id + '.xlsx'
								});
							}
						},
...
/*
        var pimage = "";
		Ext.Ajax.request({
			url: '../../server/fields/getimage.php',
			method: 'POST',
			params:{
				patient_id:data.id,
			},
...
*/
       ...
       //***AVA*** !!! AJAX SUPER TRICK !!!
		var fetchRow = function(params){
			var me = this;
			var v;
			fetchRowRequest(params,function(value){
				v = value;
			});
			return v;
		}
		var fetchRowRequest = function(params,callback){
			var me = this;
			var record;
			Ext.Ajax.request({
				url: '../../server/fields/getimage.php',
				method: 'POST',
				params: {patient_id:params},
				async:false,
				success: function(response){
					var result = Ext.JSON.decode(response.responseText);
					if (result.pimage!=null) {
						var pimage = result.pimage;
					}
					else {
						pimage = null;
					}
	  ...
	  //alert(data.pimage.toString());
		frmShowPatient= new Ext.window.Window({
			autoWidth: true,
			autoHeight: true,
			minHeight: Ext.getBody().getHeight(),
			width: Ext.getBody().getWidth() - 20,
			x: 0,
			title: "Search Result " + data.id + " - " + data.patientname + " " + data.patientlastname,
			constrain: true,
			renderTo: Ext.getBody(),
			closable: true,
			autoScroll: true,
						...
						items: [
							{
								...
								items: [{
									...
									height: 150,
									anchor:'95%',
									src: 'data:image/jpeg;base64,' + fetchRow(data.id).toString() ,
									//value: data.pimage,
										...
										items: [

											{
												...
												listeners: {
													beforerender: function(btn) {
														if(acc == 'r' ||
															(data.category_id != 31 && data.category_id !=33 && data.category_id != 34 && data.category_id != 35 &&
															data.category_id != 66)) {
...

											{
												...
												handler:function(btn){
													...
													Ext.Msg.show({
														...
														fn: function (btn) {
															if (btn == 'yes') {
																Ext.Ajax.request({
																	url: '../../server/list/patientremovals.php?act=Delete',
																	method: 'POST',
																	params:{
																		param_id:data.id,
												...
												listeners: {
													beforerender: function() {
														if(acc == 'r' ||
															(data.category_id != 57 && data.category_id !=58 && data.category_id != 59 && data.category_id != 60 &&
															data.category_id != 61 && data.category_id != 62 && data.category_id != 63 && data.category_id != 64 &&
															data.category_id != 68)
														) {
						...
						},
						{
							...
							items:[
								{
									...
									items:[

										{
											...
											listeners: {
												beforerender: function() {
												  if(data.category_id == 57 || data.category_id ==58 || data.category_id == 59 || data.category_id == 60 ||
													  data.category_id == 61 || data.category_id == 62 || data.category_id == 63 || data.category_id == 64 ||
													  data.category_id == 68) {
														this.html ='<div>' + data.removaldate + '- <strong>' + data.removal + '</strong></div>';
													}
												}
											}
										},
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'სამედიცინო ინფორმაცია_Search_' + data.id + '.xlsx'
														});
													}
												}
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'ავადმყოფობის ისტორია_Search_' + data.id + '.xlsx'
														});
													}
												}
							...
							listeners:{
								resize:function(){
									...
									frmShowPatient.setPosition(0,window.left,true);
								}
							}
						}
					]
	...
	function getData()
	{
		...
		var jsonData =Ext.decode(getParameterByName('data'));
		for (var key in jsonData) {
			if(key == 'patient_id')
				datar["id"] = jsonData['patient_id'];
			else if(key == 'birthday'){
				var dArr = jsonData['birthday'].split("-");
				if(parseInt(dArr[0]) > 0) {
					var d = dArr[2].slice(0,2);
					var m = parseInt(dArr[1]);
					var Y = dArr[0];
					datar["birthday"] = d+"."+m+"."+Y; //new Date(Y,m,d);
				}
			}
			else if(key == 'transdate'){
				var dArr = jsonData['transdate'].split("-");
				if(parseInt(dArr[0]) > 0) {
					var d = dArr[2].slice(0,2);
					var m = parseInt(dArr[1]);
					var Y = dArr[0];
					datar["transdate"] = d+"."+m+"."+Y; //new Date(Y,m,d);
				}
			}

			else
				datar[key] = jsonData[key];
		}
