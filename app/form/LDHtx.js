...
Ext.onReady(function() {
	...
	var storeHtx=Ext.create('Ext.mod.store.LDHtx',{
		model: 'Ext.mod.model.LDHtx',
		storeCount: 0,
		//buffered: true,
	...
	var rightClickGroup=Ext.create('Ext.menu.Menu',{
		...
		items:[{
			...
			handler:function(btn){
				var grid=Ext.getCmp(id+'grid_htx').getStore();
				grid.reload();
	...
	var win = Ext.create('Ext.window.Window', {
		title: 'HTx - აღრიცხვაზე მყოფ გულგადანერგილ პაციენტთა სია',
		closable: false,
		/*
		 width: 350,
		 height:400,
	...
	storeHtx.on('load', function(store, records, successful){
		win.setTitle('Htx - აღრიცხვაზე მყოფ გულგადანერგილ პაციენტთა სია - სულ ' +  store.getTotalCount());
	});
	...
	function CreateEditPatient(data){
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'ლაბოროტორია_Htx_' + data.id + '.xlsx'
						});
					}
				}
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'ინსტრუმენტალური_Htx_' + data.id + '.xlsx'
						});
					}
				}
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'მკურნალობა_HTx_' + data.id + '.xlsx'
						});
					}
				}
...

						{
							...
							handler : function(){
								...
								grid.doExcelExport({
									...
									startCell: 'B8',
									templatefile: "temp.xlsx",
									destinationfile: 'იმუნოსუპრესია_Htx_' + data.id + '.xlsx'
								});
							}
						},
		...
		frmShowPatient= new Ext.window.Window({
			...
			x: 0,
			title: "HTx " + data.id + " - " + data.patientname + " " + data.patientlastname,
			constrain: true,
			renderTo: Ext.getBody(),
			closable: true,
			autoScroll: true,
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'სამედიცინო ინფორმაცია_Htx_' + data.id + '.xlsx'
														});
													}
												}
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'ავადმყოფობის ისტორია_Htx_' + data.id + '.xlsx'
														});
													}
												}
