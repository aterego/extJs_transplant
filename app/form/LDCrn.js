...
Ext.onReady(function() {
	...
	var storeCrn=Ext.create('Ext.mod.store.LDCrn',{
	...
	var rightClickGroup=Ext.create('Ext.menu.Menu',{
		...
		items:[{
			...
			handler:function(btn){
				var grid=Ext.getCmp(id+'grid_crn').getStore();
				grid.reload();
	...
	combo.on('select', function(combo, records) {
		...
		//storeCrn.autoLoad ={ params: { start: 0, limit: p} };
		storeCrn.pageSize = p;
		var grid=Ext.getCmp(id+'grid_crn').getStore();
		grid.reload();
		storeCrn.loadPage(1);
		...
		items:[{
			...
			listeners : {
				itemdblclick: function(dv, record, item, index, e) {
					var grid = Ext.getCmp(id+'grid_crn');
	...
	storeCrn.on('load', function(store, records, successful){
		win.setTitle('CorTx - აღრიცხვაზე მყოფ რქოვანაგადანერგილ პაციენტთა სია - სულ ' +  store.getTotalCount());
	});
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'ლაბოროტორია_CorTx_' + data.id + '.xlsx'
						});
					}
				}
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'ინსტრუმენტალური_CorTx_' + data.id + '.xlsx'
						});
					}
				}
...

						{
							...
							handler : function(){
								...
								grid.doExcelExport({
									...
									startCell: 'B8',
									templatefile: "temp.xlsx",
									destinationfile: 'იმუნოსუპრესია_CorTx_' + data.id + '.xlsx'
								});
							}
						},
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'სამედიცინო ინფორმაცია_CorTx_' + data.id + '.xlsx'
														});
													}
												}
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'ავადმყოფობის ისტორია_CorTx_' + data.id + '.xlsx'
														});
													}
												}
	...
	function test(){
		...
		//Ext.getCmp(id+'grid_crn').getStore().load();
		//Ext.getCmp(id+'grid_crn').getView().refresh();
