...
requires:[
	...
	function CreateEditPatient(data){
		...
		frmShowPatient= new Ext.window.Window({
			...
			x: 0,
			title: "Liv " + data.id + " - " + data.patientname + " " + data.patientlastname,
			constrain: true,
			renderTo: Ext.getBody(),
			closable: false,
			autoScroll: true,
						...
						items:[{
							...
							items:[

								{
									xtype:'panel',
									padding: '5 0 10 0',
									html:'<div><strong>სისხლის ჯგუფი:</strong> ' + blood + ', <strong>რეზუსი:</strong> ' + rhesus + '</div>' +
									'<div><strong>დამატებითი ინფორმაცია:</strong> ' + data.info + '</div>' +
							...
							items:[

								{
									xtype:'panel',
									padding: '5 0 10 0',
									html:'<div><a class="tr_list" id="tr_list" href="#" onClick="javascript:OpenTreatmentList2(' + data.id + ','+ data.reg_id +');">მკურნალობის სია</a></div>',
									border: 0,
								},
								{
									xtype:'grid',
