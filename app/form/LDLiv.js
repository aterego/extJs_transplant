...
requires:[
	...
	var storeLDLiv=Ext.create('Ext.mod.store.LDLiv',{
		...
		var rightClickGroup=Ext.create('Ext.menu.Menu',{
			...
			items:[{
				...
				handler:function(btn){
					var grid=Ext.getCmp(id+'grid_ldliv').getStore();
						grid.reload();
	...
	combo.on('select', function(combo, records) {
		...
		storeLDLiv.pageSize = p;
		var grid=Ext.getCmp(id+'grid_ldliv').getStore();
		grid.reload();
		storeLDLiv.loadPage(1);
	}, this);
...
*/
                    ...
                    */
					...
					listeners : {
    					itemdblclick: function(dv, record, item, index, e) {
							var grid = Ext.getCmp(id+'grid_ldliv');
	...
	storeLDLiv.on('load', function(store, records, successful){
		win.setTitle('LDLiv - აღრიცხვაზე მყოფ ღვიძლის დონორთა სია - სულ ' +  store.getTotalCount());
	});
...
/*
			listeners:{
				'close':function(win){
					//Ext.getCmp(id+'grid_ldliv').getView().select(0);
					var grid = Ext.getCmp(id+'grid_ldliv');
...
*/
	...
	function test(){
		...
		//Ext.getCmp(id+'grid_ldliv').getStore().load();
		//Ext.getCmp(id+'grid_ldliv').getView().refresh();

		storeLDLiv.removeAll();
