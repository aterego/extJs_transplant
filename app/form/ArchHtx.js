...
Ext.onReady(function() {
	...
	var storeArchHtx=Ext.create('Ext.mod.store.ArchHtx',{
		model: 'Ext.mod.model.ArchHtx',
		storeCount: 0,
		//buffered: true,
	...
	var rightClickGroup=Ext.create('Ext.menu.Menu',{
		...
		items:[{
			...
			handler:function(btn){
				var grid=Ext.getCmp(id+'grid_archhtx').getStore();
				grid.reload();
	...
	combo.on('select', function(combo, records) {
		...
		//storeHtx.autoLoad ={ params: { start: 0, limit: p} };
		storeArchHtx.pageSize = p;
		var grid=Ext.getCmp(id+'grid_archhtx').getStore();
		grid.reload();
		storeArchHtx.loadPage(1);
	}, this);
	...
	var win = Ext.create('Ext.window.Window', {
		title: 'ArchHTx - არქივი გული პაციენტთა სია',
		closable: false,
		/*
		 width: 350,
		 height:400,
		...
		items:[{
			...
			listeners : {
				itemdblclick: function(dv, record, item, index, e) {
					var grid = Ext.getCmp(id+'grid_archhtx');
			...
			tbar:[
				...
				/*
				 ...
				 handler:function(btn){
				 var grid = Ext.getCmp(id+'grid_htx');
				 var model = grid.getSelectionModel().getSelection();
				 if(model[0].data != null){
				 model_edit=model[0].data;
	...
	storeArchHtx.on('load', function(store, records, successful){
		win.setTitle('ArchHtx - არქივი გული პაციენტთა სია - სულ ' +  store.getTotalCount());
	});
	...
	function CreateEditPatient(data){
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'ლაბოროტორია_ArchHtx_' + data.id + '.xlsx'
						});
					}
				}
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'ინსტრუმენტალური_ArchHtx_' + data.id + '.xlsx'
						});
					}
				}
...

				{
					...
					handler : function(){
						...
						grid.doExcelExport({
							...
							startCell: 'B8',
							templatefile: "temp.xlsx",
							destinationfile: 'მკურნალობა_ArchHTx_' + data.id + '.xlsx'
						});
					}
				}
...

						{
							...
							handler : function(){
								...
								grid.doExcelExport({
									...
									startCell: 'B8',
									templatefile: "temp.xlsx",
									destinationfile: 'იმუნოსუპრესია_ArchHtx_' + data.id + '.xlsx'
								});
							}
						},
		...
		frmShowPatient= new Ext.window.Window({
			...
			x: 0,
			title: "ArchHTx " + data.id + " - " + data.patientname + " " + data.patientlastname,
			constrain: true,
			renderTo: Ext.getBody(),
			closable: true,
			autoScroll: true,
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'სამედიცინო ინფორმაცია_ArchHtx_' + data.id + '.xlsx'
														});
													}
												}
...

												{
													...
													handler : function(){
														...
														grid.doExcelExport({
															...
															startCell: 'B8',
															templatefile: "temp.xlsx",
															destinationfile: 'ავადმყოფობის ისტორია_ArchHtx_' + data.id + '.xlsx'
														});
													}
												}
	...
	function test(){
		...
		//Ext.getCmp(id+'grid_htx').getStore().load();
		//Ext.getCmp(id+'grid_htx').getView().refresh();

		storeArchHtx.removeAll();
