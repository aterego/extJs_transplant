	...
	switch($act){
		case "View":
			...
			$query= $mysqli->query("
                             ...
                             from
                             (
                              SELECT  pat.*,pi.blood_type_id,pi.rhesus,pi.info, p0.`patient_id` as id, pc.reg_id, p0.`date` as mdate,  p0.`donor_type_id` as mdonor, p0.clinic_id as clinic, max(pc.`category_id`) as m,  max(t.treatment_id) as t, t.`date` as tdate FROM 
                              (SELECT o.* 
                                FROM `a_transplantations` o   
                                LEFT JOIN `a_transplantations` b            
                                ON o.`patient_id` = b.`patient_id` AND o.`fc` < b.`fc`
