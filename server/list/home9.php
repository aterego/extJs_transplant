...
$temp_record=array();
			...
			// count the Data Liv
			$result = $mysqli->query("SELECT COUNT( DISTINCT id) AS count 
                                        FROM
                                        (
                                        ...
                                        WHERE cat.parent_category_id Not in(29,30,45,51)  
                                        ORDER BY `p`.`id` ASC");
			...
			// count the Data Liv
			$result = $mysqli->query("SELECT COUNT(DISTINCT id) AS count , co.`name` AS name
                                        FROM
                                        (
                                          SELECT pc.`patient_id` as id,  max(pc.`category_id`) as m FROM
                                           `a_patient_category` pc
