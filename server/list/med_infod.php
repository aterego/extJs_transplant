...
if(!empty($act)){
	...
	switch($act){
		case "View":
			...
			// count the Data
			$result = $mysqli->query("SELECT COUNT(*) AS count FROM `a_transplantations` t where  t.`patient_id` = ".$_POST['param_id']);
			...
			$query= $mysqli->query("
                             ...
                             where  t.`patient_id` = ".$_POST['param_id']." order by t.`date` DESC  LIMIT $start, $limit");
