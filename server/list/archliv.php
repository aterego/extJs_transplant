	...
	switch($act){
		case "View":
			...
			$query= $mysqli->query("
                             ...
                             from
                             (
                              SELECT  pat.*,pi.blood_type_id,pi.rhesus,pi.info, p0.`patient_id` as id, pc.reg_id, p0.`date` as mdate,  p0.`donor_type_id` as mdonor, p0.clinic_id as clinic, max(pc.`category_id`) as m,  max(t.treatment_id) as t, re.removal_id, re.date as removal_date FROM 
                              (SELECT o.* 
                                FROM `a_transplantations` o   
                                LEFT JOIN `a_transplantations` b            
                                ON o.`patient_id` = b.`patient_id` AND o.`fc` < b.`fc`
                              ...
                              left join `a_patient_removals` re on re.patient_id =p0.`patient_id`
                              WHERE  `category_id` in (43,59)   group by id 
                             ) as p 

                            where p.m=59 group by p.id order by p.`patientlastname`  LIMIT $start, $limit");
			...
			if($query){
				while($r = $query->fetch_array()){
					...
					*/
					...
					$query2 = $mysqli->query("SELECT name FROM a_removals WHERE `removal_id`= ". $r['removal_id']);
					if($query2){
						$r2 = $query2->fetch_array();
						$removal = $r2['name'];
					}
					...
					$temp_record[]=array(
						...
						"removal_date"=>(abs($r['removal_date'])< 1000)? "-" : date('d.m.Y', $r['removal_date']),
