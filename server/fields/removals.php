...
if(!empty($act)){
	...
	switch($act){
		...
		case "Add":
			...
			$query= $mysqli->query("insert into a_removals (`name`) values('".$_POST['name']."')");
			if($query){
		...
		case "Update":
			$query=$mysqli->query("update a_removals set `name`='".$_POST['name']."' where removal_id=".$_POST['param_id']);
			if($query){
				$response = array( 'success'=>true, 'data'=>true );
			}else{
				$response=array('success'=>false,'data'=>false);
		...
		case "Delete":
			$query=$mysqli->query("delete from a_removals where removal_id=".$_POST['param_id']);
			if($query){
				$response = array( 'success'=>true, 'data'=>true );
			}else{
				$response=array('success'=>false,'data'=>false);
