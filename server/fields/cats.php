...
if(!empty($act)){
	...
	switch($act){
		case "View":
			$query= $mysqli->query("SELECT * FROM categories WHERE parent_category_id=0");
			if($query){
		...
		case "Update":
			...
			$query=$mysqli->query("update categories set `parent_category_id`=".$parentCat.",  `category_name`='".$_POST['category_name']."',
